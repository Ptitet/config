...
let
  niri-dynamic-float = pkgs.writers.writePython3 "niri-dynamic-float" { doCheck = false; } ''
    """
    ...
    RULES = [
        Rule([Match(title="Extension : \\(Gestionnaire de mots de passe Bitwarden\\) - Bitwarden — Zen Twilight", app_id="zen-twilight")]),
    ]
    ...
    def float(id: int):
        ...
        send({"Action": {"SetWindowHeight": {"id": id, "change": {"SetFixed": 800}}}})
        send({"Action": {"SetWindowWidth": {"id": id, "change": {"SetFixed": 650}}}})
        sleep(0.1)
...
in
{
  ...
  xdg.configFile."niri/config.kdl".source =
    config.lib.file.mkOutOfStoreSymlink "${flakeRoot}/users/jules/features/services/niri/config.kdl";
}
