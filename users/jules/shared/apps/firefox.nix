{ config, lib, ... }:
let
  cfg = config.my.apps.firefox;
in
{
  options.my.apps.firefox.enable = lib.mkEnableOption "Enables Firefox";
  ...
  config = lib.mkIf cfg.enable {
    programs.firefox = {
      ...
      policies =
        let
          locked = attrs: { Locked = true; } // attrs;
        in
        {
          ...
          EnableTrackingProtection = locked {
            Value = true;
            Cryptomining = true;
            Fingerprinting = true;
            EmailTracking = true;
